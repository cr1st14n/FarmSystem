...
class reportesController extends Controller
{
    ...
    public function reportePrueba()
    {
        ...
        $CostoTotal = 0;
        $cantidad = 0;
        $fecha0=(Carbon::createFromFormat('Y-m-d', '2018-10-30')->format('d-m-Y'));
        $ventas = venta::join('users', 'ventas.ca_cod_usu', '=', 'users.usu_ci')
            ->join('clientes', 'clientes.id', '=', 'ventas.vent_IdCliente')
            ->join('facturas', 'facturas.cod_venta', '=', 'ventas.id')
            ->select('ventas.*', 'clientes.vent_clienteNit', 'clientes.vent_clienteNombre', 'facturas.fact_numFactura')
            ->whereDate('ventas.ca_fecha', '2018-10-30')
            ->get();
        foreach ($ventas as $venta) {
            $CostoTotal = $CostoTotal + ($venta->vent_efectivoTotal);
            $cantidad = $cantidad + 1;
        ...
        $tabla="ventas";
        //return $ventas;
        $vista= view('reportes.ventasReporte1')->with("ventas", $ventas)
            ->with("tabla", "venta")
            ->with("fechaActual", $fechaActual)
            ->with("fecha0", (Carbon::createFromFormat('Y-m-d', '2018-10-30')->format('d-m-Y')))
            ->with("CostoTotal", $CostoTotal)
            ->with("cantidad", $cantidad);
        $pdf = PDF::setPaper('a4')->loadView('reportes.ventasReporte1',compact( 'fechaActual','fecha0','tabla','cantidad','CostoTotal'));
